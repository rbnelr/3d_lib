...
namespace engine {
	...
	class Display {
		...
		static void button_event (GLFWwindow* window, int button, int action, int mods) {
			Input* inp = &((Display*)glfwGetWindowUserPointer(window))->inp;
	...
	public:
		...
		//
		typedef void (* frame_proc)(Display& dsp, Input& inp, f32 dt);
		...
		static void run_frame (GLFWwindow* window) {
			auto* dsp = ((Display*)glfwGetWindowUserPointer(window));
			
			shader_manager.poll_reload_shaders(dsp->frame_i);

			auto& inp = dsp->poll_input(dsp->inp.gui_input_enabled);

			if (inp.went_down(GLFW_KEY_F11))
				dsp->toggle_fullscreen();

			bool trigger_load = inp.alt_combo('L') || dsp->frame_i == 0;
			...
			begin_imgui(&inp, dsp->dt, inp.gui_input_enabled, imgui_enabled);

			{
				...
				if (dsp->frame_i > 0) { // fps is inf, dt is 0 on frame 0
					dt_avg.update(dt, dt);
				...
				dsp->dt = max(dsp->dt, 1.0f / 20); // prevent big timestep when paused or frozen for whatever reason
			}
			...
			dsp->dt = dsp->dt_measure.frame();
		}
		static void glfw_resize_or_move_event (GLFWwindow* window) {
			auto* dsp = ((Display*)glfwGetWindowUserPointer(window));
			
			if (dsp->stop_recursion) return;
			dsp->stop_recursion = true;
			
			run_frame(window);
			
			dsp->stop_recursion = false;
		}

		void run (frame_proc frame) {
			this->frame = frame;
	...
	void run_display (Display::frame_proc frame, std::string const& title, iv2 default_size=iv2(1280,720), e_vsync_mode vsync_mode=VSYNC_ON) {

		engine::Display dsp;
		dsp.open(title, default_size, vsync_mode);
